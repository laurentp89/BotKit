...
class CloudWatchLogger {
  constructor() {
    ...
    this.logGroupName = `/botkit/${process.env.BOT_NAME}`;
    ...
    try {
      try {
        ...
        if (err.name !== "ResourceAlreadyExistsException") {
          ...
          console.warn(`Error al crear log group en Cloudwatch ${err.name}`);
        } else {
          console.warn(`Error al crear log group en Cloudwatch, ya existe el log group ${err.name}`);
        }
      }
