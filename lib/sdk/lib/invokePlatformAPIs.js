...
function getSignedJWTToken(botId) {
  ...
  return jwt.encode(
    {
      appId: appId,
      exp: Date.now() / 1000 + (jwtExpiry || 60),
    },
    apiKey,
    jwtAlgorithm || defAlg,
  );
...
module.exports = {
  post: function (url, body, callback) {
    return makeRequest(url, "post", body).nodeify(callback);
  },
  get: function (url, callback) {
    return makeRequest(url, "get").nodeify(callback);
  },
  getWithOptions: function (url, opts, callback) {
    return makeRequest(url, "get", undefined, opts).nodeify(callback);
  },
};
