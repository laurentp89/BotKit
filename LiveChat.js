...
function getPendingMessages(visitorId, ssid, last_message_id) {
  ...
  return api
    .getPendingMessages(visitorId, ssid, last_message_id, licence_id)
    .then(function (res) {
      _.each(res.events, function (event) {
        ...
        if (event.type === "message" && event.user_type !== "visitor") {
          ...
          return sdk
            .sendUserMessage(data, function (err) {
              console.log("err", err);
            })
            .catch(function (e) {
              console.log(e);
...
function connectToAgent(requestId, data, cb) {
  ...
  formdata.welcome_message =
    "Link for user Chat history with bot: " + config.app.url + "/history/index.html?visitorId=" + visitorId;
...
function onBotMessage(requestId, data, cb) {
  ...
  if (message_tone && message_tone.length > 0) {
    ...
    // condition for skipping a user message
...
function onUserMessage(requestId, data, cb) {
  ...
  if (entry) {
    ...
    if (data.message === "skipBotMessage")
      // condition for skipping a bot message
      return sdk.skipBotMessage(data, cb);
    else return sdk.sendBotMessage(data, cb);
  }
}
